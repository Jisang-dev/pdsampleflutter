...
class Receive1App extends StatefulWidget {
  Receive1App(String com) {
    commit = com;
...
class _MyAppState extends State<Receive1App> with TickerProviderStateMixin {
    ...
    return Container(
      ...
      child: Container(
        ...
        child: ListTile(
          ...
          trailing: Container(
            child: ButtonTheme(
              ...
              child: RaisedButton(
                padding: EdgeInsets.zero,
                materialTapTargetSize: MaterialTapTargetSize.shrinkWrap,
                color: confirm1 ? Colors.blue : Colors.grey,
                onPressed: _map,
                child: Container(
                  padding: EdgeInsets.all(10.0),
          ...
          subtitle: Text("버스위치가 확인되면 색상이 바뀝니다. 그때 오른쪽 버튼을 눌러 위치를 확인해주세요.",),
          onTap: () {
            setState(() {
              !confirm1 ? alert("버스 위치가 확인되지 않았습니다. 새로고침을 눌러 다시 확인해주세요. 오류 지속 시 주차부에 문의바랍니다.", 0) : alert("버스가 도착하였습니다. 위치를 확인해주세요.", 0);
            });
          },
        ),
      ),
  ...
  Widget terminalDepart() {
    return Container(
      ...
      child: Container(
        ...
        child: ListTile(
          dense: true,
          leading: Icon(Icons.looks_one, color: !confirm2 ? Colors.red : Colors.green,),
          title: Text("승차 완료, 출발 (앱종료)", style: TextStyle(fontSize: 20),),
          subtitle: Text("모두 승차하고, 버스가 주차장을 떠날 때 누릅니다.",),
          onTap: () {
            !confirm2 ? alert("버스에 승객이 모두 승차하였고, 주차장을 빠져나왔습니까?", 4) : alert("버스가 주차장을 빠져나온 것으로 입력되어 있습니다. 아직 빠져나오지 않으셨다면 '네'를 눌러주세요.", 9);
          },
        ),
      ),
    );
  ...
  Widget finish() {
    return ListTile(
      title: confirm2 ? Text("수고하셨습니다!", style: TextStyle(fontSize: 14), textAlign: TextAlign.center,) : null,
    );
  }
